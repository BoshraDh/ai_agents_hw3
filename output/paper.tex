...
\begin{figure}[H]
    ...
    \fbox{%
        \parbox[c][7cm][c]{0.85\textwidth}{%
            \centering
            \textit{[Architecture diagram placeholder]}\\[1em]
            \small Place \texttt{images/arch\_comparison.png} in the same
            folder as this \texttt{.tex} file, then replace this block with
...
\begin{figure}[H]
    ...
    % ----------------------------------------------------------
    % Replace the \fbox placeholder below with:
    %   \includegraphics[width=0.75\textwidth]{images/performance_chart.png}
    % ----------------------------------------------------------
    \fbox{%
        \parbox[c][7cm][c]{0.75\textwidth}{%
            \centering
            \textit{[Radar / bar chart placeholder]}\\[1em]
            \small Place \texttt{images/performance\_chart.png} in the
            \texttt{images/} folder and replace this block with
            \texttt{\textbackslash includegraphics}.
        }%
    }
